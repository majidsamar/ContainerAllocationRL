...
#run parameters
MAX_NUM_CONTAINERS_PER_EPISODE = 18

TRAIN_STAGE_NUM_CONTAINERS_LIST = [10, 15, 16, 17, 18]
TRAIN_STAGE_INCREASE_EPISODE_LIST = [500, 600, 800, 900, 1001]
# episode : (0 to 100) -> 10, (101 to 200) -> 15, (201 to 250) -> 16, (251, 270) -> 17, (271, end) -> 18
# TRAIN_STAGE_NUM_CONTAINERS_LIST = [10, 15, 16, 17, 18]
# TRAIN_STAGE_INCREASE_EPISODE_LIST = [100, 200, 250, 270, 301]
...
MODEL_PATH =f'ContainerAllocationRL/topViewIsSorted_varQ/outputs/model_{rsn}_topView_isSorted_varQ_{BAYS}{ROWS}{TIERS}_{NUM_EPISODES}_{datetime.now().strftime("%Y_%m_%d__%H_%M")}.mdl'
TRAIN_LOSS_REWARD_PATH =f'ContainerAllocationRL/topViewIsSorted_varQ/outputs/loss_reward_{rsn}_{datetime.now().strftime("%Y_%m_%d__%H_%M")}.csv'
TEST_OPERATION_PATH =f'ContainerAllocationRL/topViewIsSorted_varQ/outputs/test_{rsn}_{datetime.now().strftime("%Y_%m_%d__%H_%M")}.csv'
...
class ContainerYardEnv:
    def __init__(self, bays=BAYS, rows=ROWS, tiers=TIERS):
        ...
        self.rows = rows
        self.tiers = tiers
        self.queue_length = MAX_NUM_CONTAINERS_PER_EPISODE
    ...
    @logger.log_time
    def reset(self,queue_container_count, yard_container_count,cover_tier0=False):
        ...
        # generating q_container_count number of contianers and paddig the rest with 0 to  Q with the length of MAX_NUM_CONTAINERS_PER_EPISODE
        self.container_queue = [self.generate_container() if i < queue_container_count else 0 for i in range(MAX_NUM_CONTAINERS_PER_EPISODE)]
        # self.container_queue = [self.generate_container() for _ in range(MAX_NUM_CONTAINERS_PER_EPISODE)]
    ...
    def get_state(self):
        ...
        future_containers = np.pad(
            future_containers,
            (0, MAX_NUM_CONTAINERS_PER_EPISODE - len(future_containers)),
            'constant'
        )
...
def run_dqn():
    ...
    for episode in range(NUM_EPISODES):
        num_containers = TRAIN_STAGE_NUM_CONTAINERS_LIST[train_stage] #= [10, 15, 16, 18]
        state = env.reset(queue_container_count=num_containers,
                          yard_container_count=int(env.tiers * env.rows * env.bays * INITIAL_YARD_OCCUPIED_RATIO),
                          cover_tier0=FILL_TIER0_AS_INITIALIZATION)
        total_reward = 0
        episode_loss = 0
        loss_count = 0

        for _ in range(num_containers): #MAX_NUM_CONTAINERS_PER_EPISODE
        ...
        print(f"Episode {episode},  Stage: {train_stage}, len(Q):{num_containers}  Loss Avg:{avg_loss:.4f},  Reward = {total_reward}, Epsilon = {agent.epsilon:.4f}")
        if episode == TRAIN_STAGE_INCREASE_EPISODE_LIST[train_stage]: #= [100, 200, 250, 270] , #= [10, 15, 16, 18]
            train_stage +=1
...
def run_test_agent(model_path):
    ...
    for test in range(TEST_EPISODES):
        ...
        state = env.reset(yard_container_count=int(env.tiers * env.rows * env.bays * INITIAL_YARD_OCCUPIED_RATIO),
                          queue_container_count=MAX_NUM_CONTAINERS_PER_EPISODE,
                          cover_tier0=False)
        for stp in range(MAX_NUM_CONTAINERS_PER_EPISODE):  # multiple container per episode
            action = agent.select_action(state=state)

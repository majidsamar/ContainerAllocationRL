...
MODEL_PATH =f'ContainerAllocationRL/3d_basic/outputs/model_basic_{BAYS}{ROWS}{TIERS}_{NUM_EPISODES}_{datetime.now().strftime("%Y_%m_%d__%H_%M")}.mdl'
...
def run_test_agent(model_path):
    ...
    for test in range(TEST_EPISODES):
        ...
        for stp in range(NUM_CONTAINERS_PER_EPISODE):  # multiple container per episode
            ...
            print(f" Test {test}: Placed container at b,r,t:({bay}, {row} ,{tier}), Reward: {reward}")
            state=next_state

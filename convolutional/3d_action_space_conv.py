...
MODEL_PATH =f'ContainerAllocationRL/convolutional/outputs/model_conv_{BAYS}{ROWS}{TIERS}_{NUM_EPISODES}_{datetime.now().strftime("%Y_%m_%d__%H_%M")}.mdl'
TRAIN_LOSS_REWARD_PATH =f'ContainerAllocationRL/convolutional/outputs/loss_reward_{datetime.now().strftime("%Y_%m_%d__%H_%M")}.csv'
TEST_OPERATION_PATH =f'ContainerAllocationRL/convolutional/outputs/test_{datetime.now().strftime("%Y_%m_%d__%H_%M")}.csv'

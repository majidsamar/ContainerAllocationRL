...
MODEL_PATH =(f'convolutional/models/conv.mdl')
...
class ContainerYardEnv:
    ...
    def get_valid_actions_for_state(self, state):
        ...
        #
        # yard = yard_flat.reshape(self.tiers, self.rows, self.bays)
        # # yard = np.transpose(yard, (2, 1, 0))
        #
        # # find the first empty tier in each column
        # occupied = yard > 0
        # empty = ~occupied
        #
        # # sum along tiers to get current stack height per (bay, row)
        # stack_heights = np.argmax(empty, axis=0)  # First 0 along tiers → tier to place
        # full_columns = ~np.any(empty, axis=0)  # True if column is full
        #
        # # mask out full columns
        # valid_mask = ~full_columns
        #
        # # Get coordinates of valid placements
        # bay_indices, row_indices = np.where(valid_mask)
        # tier_indices = stack_heights[bay_indices, row_indices]
        #
        # # eliminate floating containers (tier > 0 but lower tier is still empty)
        # floating_mask = (tier_indices > 0) & (yard[tier_indices - 1 , row_indices, bay_indices ] == 0)
        # bay_indices = bay_indices[~floating_mask]
        ...
        # tier_indices = tier_indices[~floating_mask]
        #
        # # convert (b, r, t) → flat action index
        # actions = tier_indices * (self.bays * self.rows) + row_indices * self.bays + bay_indices
        # return actions.tolist()
...
#         super(DQN, self).__init__()
#
#         #  note that TIERS is the depth
#         self.conv1 = nn.Conv2d(TIERS, 16, kernel_size=2, stride=1, padding=1)
#         self.conv2 = nn.Conv2d(16, 32, kernel_size=2, stride=1, padding=1)
#
#         # after flattening the output of the second convolution layer, its shape is:
#         # (32 * BAYS * ROWS) now adding the queue size  --> (32 * BAYS * ROWS + num_containers)
#         self.feature_size = 32 * BAYS * ROWS + num_containers
#         self.fc1 = nn.Linear(self.feature_size, 64)
#
#         # output_dim: BAYS * ROWS * TIERS
#         self.fc2 = nn.Linear(64, output_dim)
#
#     def forward(self, x):
#         #  x is a concatenated 1d vector of yard (3d reshaped into 1d) and queue
#         #  TIER * ROW * BAYS + Queue_Length
#         yard = x[:, :BAYS * ROWS * TIERS].view(-1, TIERS, ROWS, BAYS) # for reshaping from 1D into 3D
#         queue = x[:, BAYS * ROWS * TIERS:]
#
#         yard = F.relu(self.conv1(yard)) # output shape of layer Conv1 : 1, 16, ROWS, BAYS
#         yard = F.relu(self.conv2(yard)) # output layer 2 : Dimension : 1 , 32 , ROWS, BAYS
#
#         yard = yard.view(yard.size(0), -1)  #flatten all dimensions except batch for FC layer input
#         combined_features = torch.cat((yard, queue), dim=1) # Dimension : 32 * ROWS * BAYS + Queue_len
#
#         x = F.relu(self.fc1(combined_features))
#         x = self.fc2(x)
#         return x
